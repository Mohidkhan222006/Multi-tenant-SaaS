...
export default async function InvitePage({
  ...
  if (!invite) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 font-sans text-center">
        <div className="max-w-md border border-slate-900 bg-slate-900/30 rounded-2xl p-8">
  ...
  if (invite.status !== 'pending') {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 font-sans text-center">
        <div className="max-w-md border border-slate-900 bg-slate-900/30 rounded-2xl p-8">
          <h2 className="text-xl font-bold text-slate-100">Invite Already Accepted</h2>
          <p className="text-sm text-slate-500 mt-2">
  ...
  if (invite.expiresAt < new Date()) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 font-sans text-center">
        <div className="max-w-md border border-slate-900 bg-slate-900/30 rounded-2xl p-8">
          <h2 className="text-xl font-bold text-rose-400">Invitation Expired</h2>
          <p className="text-sm text-slate-500 mt-2">
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-slate-950 px-4 select-none relative overflow-hidden font-sans">
      ...
      <div className="w-full max-w-md relative z-10 text-center">
        <div className="inline-flex w-12 h-12 rounded-2xl bg-gradient-to-tr from-violet-600 to-indigo-600 items-center justify-center font-extrabold text-lg text-white shadow-xl shadow-violet-950/20 mb-6">
          A
        </div>
